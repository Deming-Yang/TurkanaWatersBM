...
save(post.lw.evp.k, file = "out/post.lw.evp.k.RData")

load("out/post.lw.evp.k.RData")

post.lw.evp.k$BUGSoutput$summary

# check rhat for convergence: Rhat < 1.01 means ample sampling with good convergence
post.lw.evp.k$BUGSoutput$summary[,8]

# check the density of parameters
denplot(as.mcmc(post.lw.evp.k))

# check trace plot for convergence
mcmc_trace(as.mcmc(post.lw.evp.k), pars = c("intc", "sl", "k", "x"))
