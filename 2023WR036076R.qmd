...
The code was developed in R (ver. 4.3.2) calling [JAGS (Just Another Gibbs Sampler)](https://sourceforge.net/projects/mcmc-jags/), a program for Bayesian hierarchical models. Please install JAGS before using this notebook. 

```{r}
#| label: setup
#| cache: false

pacman::p_load(here, quarto, knitr, tidyverse, kableExtra, lubridate, ggthemes, scale, deeptime, patchwork, coda, R2jags, bayestestR, bayesplot, mcmcplots)
```
...
# parameters, input data list, and driver conditions are also used for sensitivity tests below 
...
Posterior distributions for k sensitivity test 

Posterior distributions for input isotope sensitivity test 

Posterior distributions for precipitation isotope sensitivity test 

Posterior distributions for lake isotope sensitivity test 
